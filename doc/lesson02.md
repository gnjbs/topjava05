...
## Домашнее задание HW02
    ...
    - Сделать реализацию слоев приложения для функциональности "еда":
      ...
      - после авторизации пользователь попадает в контроллер 
        ...
        После подключения Spring-MVC все методы контроллера будут доступны через http. 
        Пока обращаемся к ним через сервлет, но контроллер проектируем так, что дальше работаем с ним без сервлета.
...
---------------------
### Подсказки по HW02 (лучше сначала сделать самостоятельно, а потом проверить на соответствие)
...
Т.к. контроллер позволяет управлять ТОЛЬКО своей едой, userId в параметрах методов контроллера НЕТ. 
