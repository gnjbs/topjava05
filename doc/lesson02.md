...
## Домашнее задание HW02
    Сделать реализацию слоев приложения и зарефакторить InMemoryUserMealRepositoryImpl для функциональности "еда":
    - API должна удовлетворять все потребности демо приложения и ничего лишнего (см. http://topjava.herokuapp.com/).
    - после авторизации пользователь попадает в контроллер  (его id находится в LoggedUser.id()) 
