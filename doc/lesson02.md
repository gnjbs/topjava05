...
## Домашнее задание HW02
    Сделать реализацию слоев приложения и InMemoryUserMealRepositoryImpl для функциональности "еда":
    - API должна удовлетворять все потребности демо приложения и ничего кроме этого (см. http://topjava.herokuapp.com/).
    - после авторизации пользователь попадает в контроллер  (для нас его id берем в LoggedUser.id()) 
    - ему принадлежит еда и он может управлять ТОЛЬКО СВОЕЙ едой
...
Optional 
  ...
  - Сделать инициализацию Spring и вызов UserMealRestController из MealServlet 
     (НЕ менять в pom.xml, работаем со spring-context, в дальнейшем мы будем работать с контроллерами, без сервлетов).
