...
## Домашнее задание HW02
    ...
    - Сделать реализацию слоев приложения для функциональности "еда":
      ...
      - после авторизации пользователь попадает в контроллер 
        ...
        пока (в Optional) работаем через сервлет, но контроллер проектируем так, что дальше работаем с ним без сервлета
...
Optional 
    ...
    - Сделать инициализацию Spring и вызов UserMealRestController из MealServlet (НЕ менять в pom.xml, работаем со spring-context)
